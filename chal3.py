...
def strxor(bytes_, b):
    return ''.join([ chr(x^b) for x in bytes_])

def decrypt(bytes_):
    """ decrypt a hex-encoded string that has been xored with a single character.  """
    candidates = [ strxor(bytes_, b) for b in range(256) ]
    return max(candidates, key=fitness)

if __name__ == "__main__":
    from sys import argv
    print(decrypt(bytearray.fromhex('1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736')))
