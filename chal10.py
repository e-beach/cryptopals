...
def CBC_encrypt(message, iv=IV, key=chal7.SUBMARINE_KEY):
    message = chal9.pad16(message)
    message = as_bytes(message)
    result = []
    last = iv
    for blck in chal8.blocks(message, 16):
        last = chal7.ECB_encrypt(chal3.strxor(blck, last), key)
        result.append(last)
    return b''.join(result)

def CBC_decrypt(message, iv=IV, key=chal7.SUBMARINE_KEY):
    result = []
    last = iv
    for blck in chal8.blocks(message, 16):
...
def pad_EBC_encrypt(message, key=chal7.SUBMARINE_KEY):
    message = chal9.pad16(message)
