...
def pad_CBC_encrypt(message, iv=IV, key=c.SUBMARINE_KEY, padding=True):
    if padding:
        message = c.pad16(message)
    message = as_bytes(message)
    result = []
    last = iv
    for blck in c.chunks(message, 16):
        last = chal7.ECB_encrypt(chal3.strxor(blck, last), key)
        result.append(last)
    return b''.join(result)

def CBC_decrypt(message, iv=IV, key=c.SUBMARINE_KEY):
    result = []
    last = iv
...
def pad_ECB_encrypt(message, key=c.SUBMARINE_KEY):
...
if __name__ == "__main__":
    ...
    assert pad_CBC_encrypt(CBC_decrypt(content), padding=False) == content
